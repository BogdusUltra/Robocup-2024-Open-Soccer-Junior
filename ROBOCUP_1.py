...
cv2.circle(mask1, (235, 245), insideCircle, 0, -1)
...
oppGoal = validFlag(flag_OppGoal, 0.2, 0.1)
side = 'None'
count_side = 0
...
dist_ball = 0
flag_ball = False
ball = validFlag(flag_ball, 0.1, 0.1)
...
def robot_coord(hsv):
    ...
    if sum_area1 < 1000 and sum_area2 < 1000:
        flag_coord = False
    else:
        flag_coord = True
        if sum_area1 > sum_area2:
    ...
    if ANGEL_OPPGOAL < 0:
        ANGEL_OPPGOAL += 360
    # print(x_c, x_d, tg, a, ANGEL_OPPGOAL)
    return hsv2
...
def walls(dist=300, kp=1.0, kd=1.0, max_u=100):
...
e_old_g = 0
def goal_limit(dist, angle, min_dist=150, kp=3, kd=3, max_u=100):
        ...
        while 1:
            ...
            if a != '$':
                ...
                if len(inn) == 6:
                    ...
                    X_2_robot = int(inn[4:5])
                    Y_2_robot = int(inn[5:6])
                break
            if t + 0.02 < time.time():
                break
        port.reset_input_buffer()

def intersect(sp, min=37, max=100):
    if 0< sp < min:
        sp = min
    if 0 > sp > -min:
        sp = -min
...
red_len1, red_len2 = 0, 0
e_old_redzone = 0
def red_zone(min_dist = 370, kp=10, kd=10, max_u = 100):
    global  red_len1, red_len2, e_old_redzone, sp1, sp2, sp3, sp4
    x = (X1 + X2)//2
    y1, y2 = 350, Y1 + Y2 - 350
    delta_x = x - X1
    delta_y1 = y1 - Y1
    delta_y2 = y2 - Y1
    red_len1 = (delta_x**2 + delta_y1**2)**0.5
    red_len2 = (delta_x ** 2 + delta_y2 ** 2) ** 0.5
    if red_len1 < min_dist:
        e = min_dist - red_len1
        u = round(e * kp + (e - e_old_redzone) * kd)
        u = intersect(u, 0, max_u)
        s1, s2, s3, s4 = moving(-ANGEL_OPPGOAL, u)
        sp1+=s1
        sp2+=s2
        sp3+=s3
        sp4+=s4
        e_old_redzone = e
    elif red_len2 < min_dist:
        e = min_dist - red_len2
        u = round(e * kp + (e - e_old_redzone) * kd)
        u = intersect(u, 0, max_u)
        s1, s2, s3, s4 = moving(-ANGEL_OPPGOAL, -u)
...
while 1:
        ...
        if role == "defend":
            ...
            if valid_flag_HomeGoal:
                ...
                if state == 1:
                    if valid_flag_ballInDribler:
                        state = 4
                    elif valid_flag_ball and dist_HomeGoal < 200:
                        state = 2
                    elif valid_flag_robots and dist_HomeGoal < 200:
                        state = 2
                    else:
                        if flag_coord:
                        ...
                        pd2(dist=dist_HomeGoal, target=185, max_u=98, center_e=40, e_min=12, kp=3, kd=3)
                    ...
                    if valid_flag_ballInDribler:
                        ...
                        pd2(dist=dist_HomeGoal, target=165, max_u=98, center_e=30, kp=3 * kf, kd=5 * kf, e_min=16)
                    elif valid_flag_robots:
                        ...
                        target = 0
                        kf = 0.5
                        pd3(ang=angle_robot, target=target, max_u=60, e_min=6, kd=3 * kf, kp=2 * kf, center_e=20)
                        pd2(dist=dist_HomeGoal, target=160, max_u=60, center_e=30, kp=1 * kf, kd=3 * kf, e_min=12)
                    ...
                    if valid_flag_ballInDribler:
                        ...
                        sp1, sp2, sp3, sp4 = moving(angel_ball + 10, -40)
                        pd1(ang=angel_ball, target=350, max_u=80, center_e=10, e_min=3, kp=0.8, kd=0.8)
                    ...
                    if flag_ballInDribler:
                        if type_kick == 1:
                            ...
                            Pd1.validTimeTrue = 0.2
                            if flag_OppGoal:
                                pd1(ang=angel_OppGoal, target=180, max_u=40, center_e=7, e_min=7, kp=0.5, kd=0.5)
                                valid_flag_pd1 = Pd1.getValid(flag_pd1)
                    ...
                    dribler = 0
                    if (Y1 + Y2 - 800) * (Y2 - 400) != 0:
                        t = 0.8 / (Y1 + Y2 - 800) * (Y2 - 400)
                    # print(t)
                    if timerState + (0.7 - t) < time.time():
                        if turn == -1:
            ...
            if state == 1:
                ...
                if valid_flag_ballInDribler:
                    ...
                    pass
                    # sp1, sp2, sp3, sp4 = moving(angel_ball + 8, -35)
                elif flag_ball:
                    dribler = 10
                    if dist_ball < 165:
                        # sp = round(dist_ball / 240 * 20)
                        sp1, sp2, sp3, sp4 = moving(angel_ball + 350, -40)
                        if dist_ball < 130:
                            sp1, sp2, sp3, sp4 = moving(angel_ball + 10, -35)
                        pd1(ang=angel_ball, target=350, max_u=100, center_e=90, e_min=3, kp=1, kd=1)
                    else:
                        sp1, sp2, sp3, sp4 = moving(angel_ball + 10, -90)
                        pd1(ang=angel_ball, target=350, max_u=100, center_e=90, e_min=2, kp=1, kd=1)
                else:
                    state = 4
                ...
                if flag_ballInDribler:
                    ...
                    if type_kick == 1:
                        ...
                        if flag_OppGoal:
                            Pd1.validTimeTrue = 0.2
                            # pd_XY(x1=x1, y1=0, x2=x2, y2=Y1+Y2, kp=0.1, kd=0.1, max_u=55)
                            pd1(ang=angel_OppGoal, target=0, max_u=37, center_e=7, e_min=7, kp=1, kd=1)
                            valid_flag_pd1 = Pd1.getValid(flag_pd1)
                            if valid_flag_pd1:
                                sp1, sp2, sp3, sp4 = 0, 0, 0, 0
                                zummer = 1
                            ...
                            walls(dist=1200, kp=0.4, kd=0.4, max_u=50)
                        ...
                        if flag_coord:
                            ...
                            if Y1 < y_c:
                                pd_XY(x1=x1, y1=0, x2=x2, y2=Y1+Y2, kp=0.1, kd=0.1, max_u=55)
                            if flag_OppGoal:
                                pd1(ang=angel_OppGoal, target=180, max_u=37, center_e=5, e_min=5, kp=1, kd=1)
                                valid_flag_pd1 = Pd1.getValid(flag_pd1)
                                if valid_flag_pd1 and (flag_pdXY or Y1>y_c):
                                    state = 3
                                    timerState = time.time()
                                ...
                                pd_XY(x1=0, y1=0, x2=X1+X2, y2=y_c, kp=0.1, kd=0.1, max_u=55)
                        else:
                            walls(dist=1200, kp=0.4, kd=0.4, max_u=60)
                else:
                    side = 'None'
                    type_kick = 0
                    # role = "defend"
                ...
                ballAngel, ballDist, flag_ball = findBall(hsv)
                valid_flag_ball = ball.getValid(flag_ball)
                if valid_flag_ball:
                    state = 1
                if flag_coord:
                    pd_XY(x1=x_c-100, y1=y_c-100, x2=x_c+100, y2=y_c+100, kp=0.5, kd=0.5, max_u=80)
                else:
                    walls(dist=800, kp=0.2, kd=0.2)
                ...
                if state_2_robot == 0:
                    ...
                    if x < x_c:
                        ...
                        x2 = (X1 + X2) // 3 - 200
                    if flag_coord:
                        pd_XY(x1=x1, y1=y_c, x2=x2, y2=y_c-1000, kp=0.5, kd=0.5, max_u=80)
                    else:
                        walls(dist=800, kp=0.2, kd=0.2)
    ...
    if draw == True:
        ...
        display_frame = cv2.putText(display_frame, "X1:" + str(round(X1)), (shapeX // 2 + 320, shapeY // 2+30), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 0, 255), 2)
        display_frame =  cv2.putText(display_frame, "X2:" + str(round(X2)), (shapeX // 2 - 220, shapeY // 2+30), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 0, 255), 2)
        ...
        if first_start:
            ...
            if home_col == 'yel':
                col1 = (255, 0, 0)
                col2 = (0, 255, 255)
                cv2.putText(play_frame, str(round(red_len2)),
                            (50, 30),
                            cv2.FONT_HERSHEY_SIMPLEX, 0.4,
                            (255, 255, 255), 2)
                cv2.putText(play_frame, str(round(red_len1)),
                            (50, y-15),
                            cv2.FONT_HERSHEY_SIMPLEX, 0.4,
                            (255, 255, 255), 1)
            else:
                ...
                cv2.putText(play_frame, str(round(red_len2)),
                            ...
                            (255, 255, 255), 2)
            if flag_coord == False:
                col1, col2 = (50, 50, 50), (50, 50, 50)
            cv2.rectangle(play_frame, (40, 15), (x - 40, 30), col1, -1)
            cv2.rectangle(play_frame, (40, y - 30), (x - 40, y - 15), col2, -1)

            if xc and yc != 0:
                x_0, y_0 = round(X1 / xc * x), round(Y1 / yc * y)
                x_1, y_1 = round(X_2_robot / 9 * x), round(Y_2_robot / 9 * y)
            ...
            cv2.line(display_frame, (shapeX//2+80, shapeY//2+30), (shapeX//2 + 80 + x, shapeY//2+30 - y), (255, 255, 0), 1)
        ...
        robot.text_to_frame(display_frame, "FPS: " + str(fps) + " mes:" + str(message) + " but:" + str(but1)+str(but2) + " 2_r:" +str(state_2_robot) , 5, 20)
        robot.text_to_frame(display_frame, "start:" + str(flag_start) + " stop:" + str(flag_stop) + " lost:" + str(flag_lost) + " role:" + str(role) + " state:" + str(state) + " kick:" + str(type_kick), 5, shapeY+30+20)
